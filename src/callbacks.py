...
class LogPredictionCallback(Callback):
    ...
    def on_test_epoch_end(self, trainer, pl_module):
        ...
        num_classes = pl_module.num_classes if pl_module.hparams['Modeltype'] == "GlycoModel" and pl_module.num_classes == 3 else "2 " + str(list(trainer.datamodule.config.classes.keys())[1]) + "-Linked"

        # Save test predictions to csv
        self.test_preds = pd.DataFrame(zip(preds, pred_classes, ys), columns=["Score", "Pred_class", "Real_class"])
        self.test_preds.to_csv(self.out_path / f"{pl_module.hparams['Modeltype']}_{num_classes}_test_preds.tsv", sep='\t', index=False)
